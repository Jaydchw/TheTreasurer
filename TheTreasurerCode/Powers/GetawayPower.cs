...
public class GetawayPower : TheTreasurerPower
{
    ...
    {
        if (!Owner.IsPlayer || Owner.Player == null || play.Card.Owner != Owner.Player || play.Card.Type != CardType.Power)
        {
        ...
        if (target != null)
        {
            _ = CardEnchantApi.TryApply<Swift>(target, 2);
        }
